...
foreach (float start in new[] { -2f, 0f, 1f, 7.5f })
    foreach (float diff in new[] { -1f, -.5f, -.25f, 0f, .25f, .5f, 1f })
        require(Assets.Scripts.GridMath.IsElevationDiffOk(start, start + diff) == (diff == 0 || Mathf.Abs(diff) == .5f), "Exact permitted differences");
require(!Assets.Scripts.GridMath.IsElevationDiffOk(float.NaN, 0), "NaN rejected");
require(!Assets.Scripts.GridMath.IsElevationDiffOk(float.PositiveInfinity, float.PositiveInfinity), "Infinity rejected");
var ramp = new float[,] { { 1, 1.5f, 2, 2.5f } };
require(Assets.Scripts.GridMath.TestElevationLine(ramp, 0, 0, 0, 3), "Climbing several half steps");
require(Assets.Scripts.GridMath.TestElevationLine(ramp, 0, 3, 0, 0), "Descending several half steps");
ramp[0, 1] = 2;
require(!Assets.Scripts.GridMath.TestElevationLine(ramp, 0, 0, 0, 3), "Intermediate cliff");
ramp[0, 3] = 1;
require(!Assets.Scripts.GridMath.TestElevationLine(ramp, 0, 0, 0, 3), "Equal endpoints do not bypass cliff");
require(!Assets.Scripts.GridMath.TestElevationLine(ramp, 0, 0, 1, 0), "Bounds");
require(!Assets.Scripts.GridMath.TestElevationLine(null, 0, 0, 0, 0), "Missing heights");
var heights = new float[7, 7];
var exists = new bool[7, 7];
for (int r = 0; r < 7; r++) for (int c = 0; c < 7; c++) { heights[r, c] = (r + c) * .5f; exists[r, c] = true; }
require(Assets.Scripts.GridMath.TestElevationLine(heights, 0, 0, 2, 4), "Ordered shallow ray (not endpoint comparison)");
require(!Assets.Scripts.GridMath.TestElevationLine(heights, 0, 0, 3, 3), "Diagonal full step");
for (int r = 0; r < 7; r++) for (int c = 0; c < 7; c++) heights[r, c] = Mathf.Max(r, c) * .5f;
require(Assets.Scripts.GridMath.TestElevationLine(heights, 0, 0, 6, 6), "Diagonal half steps");
exists[3, 3] = false;
require(!Assets.Scripts.GridMath.TestElevationLine(heights, 0, 0, 6, 6, exists), "Missing intermediate cell");
...
for (int r1 = 0; r1 < 7; r1++) for (int c1 = 0; c1 < 7; c1++)
    for (int r2 = 0; r2 < 7; r2++) for (int c2 = 0; c2 < 7; c2++)
        require(Assets.Scripts.GridMath.TestElevationLine(heights, r1, c1, r2, c2, exists) ==
            Assets.Scripts.GridMath.TestElevationLine(heights, r2, c2, r1, c1, exists), "Ray symmetry");
...
foreach (var guid in UnityEditor.AssetDatabase.FindAssets("t:LevelMarkerTile", new[] { "Assets/Levels/Markers/Elevation" }))
{
...
var palette = UnityEditor.AssetDatabase.LoadAssetAtPath<GameObject>("Assets/Levels/Palettes/Elevation Palette.prefab")
