...
public static class CastableOutlineChecks
{
    ...
    {
        Require(checks == null, "Outline checks are already running.");
        Directory.CreateDirectory(Output);
        File.WriteAllText(Output + "/Results.txt", "RUNNING");
        CheckGeometry();
        checks = CheckRendering();
    ...
    private static void Tick()
    {
        try
        {
            ...
            File.WriteAllText(Output + "/Results.txt", "PASS: 512 grid masks; joined bands, holes, diagonal contacts, narrow corridors, empty grids; GPU fade, pixel cells, seams and sorting; mesh reuse and cleanup.");
            Debug.Log("CASTABLE OUTLINE CHECKS PASSED");
        }
        catch (Exception exception)
        {
            File.WriteAllText(Output + "/Results.txt", "FAIL: " + exception);
    ...
    public static string CheckLifecycle()
    {
        ...
        var outline = game.gridParent.GetComponentInChildren<CastableOutline>();
        ...
        Require(new SerializedObject(game).FindProperty("castableOutline").objectReferenceValue == outline, "Scene reference must be connected.");
        ...
        try
        {
            ...
            Require(Mathf.Abs(mesh.bounds.center.x - center.x - game.spacing) < .001f, "Blink arrival must move the outline.");
            player.r = -1;
            game.MakeCastable();
    ...
    {
        var image = WorldRenderingChecks.RenderCamera(camera, 320, 320);
