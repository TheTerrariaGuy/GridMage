...
public static class TilemapLevelSetup
{
    ...
    public static void Create()
    {
        ...
        var backgroundTile = AssetDatabase.LoadAssetAtPath<UnityEngine.Tilemaps.Tile>(Folder + "/Background Floor.asset");
        if (backgroundTile == null)
        {
            ...
            AssetDatabase.CreateAsset(backgroundTile, Folder + "/Background Floor.asset");
        }
        ...
        foreach (Vector2Int cell in new[] { new Vector2Int(10, 10), new Vector2Int(10, 15),
            new Vector2Int(15, 10), new Vector2Int(10, 13), new Vector2Int(13, 10) })
            if (cell.x < cols && cell.y < rows && (cell.x != player.c || cell.y != player.r))
                level.logic.SetTile(new Vector3Int(cell.x, -cell.y, 0), enemy);
        ...
        LevelSpawnKind spawn = LevelSpawnKind.None, float interval = 0f)
    {
        ...
        if (spawn == LevelSpawnKind.Enemy)
            marker.enemy = AssetDatabase.LoadAssetAtPath<EnemyData>("Assets/Scripts/ScriptableObjects/Enemies/Normal.asset");
    ...
    {
        const string path = Folder + "/Level Palette.prefab";
        if (AssetDatabase.LoadAssetAtPath<GameObject>(path) != null) return;
        GridPaletteUtility.CreateNewPalette(Folder, "Level Palette", GridLayout.CellLayout.Rectangle,
