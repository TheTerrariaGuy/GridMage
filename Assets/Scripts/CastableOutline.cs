...
public sealed class CastableOutline : MonoBehaviour
{
    ...
    public void Rebuild(bool[,] grid, float spacing, float offset)
    {
        lastGrid = grid;
        lastSpacing = spacing;
        lastOffset = offset;
    ...
    {
        if (this != null && isActiveAndEnabled) Rebuild(lastGrid, lastSpacing, lastOffset);
    }
#endif
    ...
    private static bool IsCastable(bool[,] grid, int row, int col) => row >= 0 && col >= 0 &&
        row < grid.GetLength(0) && col < grid.GetLength(1) && grid[row, col];
